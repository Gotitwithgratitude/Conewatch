...
function cameraFollow(){
  ...
  if(S.navigating){
    ...
    map.easeTo({center:[S.pos.lng,S.pos.lat],
      zoom:S.navZoom||16.6,pitch:60,   // steady zoom = authentic, no speed-tier jumping
...
function showTutStep(){
  ...
  if(r){
    ...
    // put the card opposite the highlighted element so it never covers it
    if(r.top < window.innerHeight*0.5){ card.style.top=Math.min(r.bottom+18, window.innerHeight-230)+"px"; card.style.bottom="auto"; }
    else { card.style.bottom="40px"; card.style.top="auto"; }
  } else {
    ring.style.left="50%"; ring.style.top="-60px"; ring.style.width="0"; ring.style.height="0";
    card.style.bottom="40px"; card.style.top="auto";
  }
...
function openSat(lat,lng,name){
  ...
  satMapObj.on("load",function spin(){
    const step=()=>{ if(!satMapObj||touched)return; satMapObj.setBearing(satMapObj.getBearing()+0.1); orbitRAF=requestAnimationFrame(step); };
    step();
  });
}
...
function openDriveTour(){
  ...
  tourMap=new maplibregl.Map({container:"driveMap",
    ...
    center:co[0],zoom:16.5,pitch:70,bearing:_brg(co[0],co[1]),attributionControl:true,interactive:true});
  tourMap.on("load",()=>{
    try{ tourMap.setTerrain({source:"dem",exaggeration:1.35}); }catch(e){}
...
function _tourRender(){
  ...
  tourMap.jumpTo({center:pos,bearing:st.curBrg,pitch:70,zoom:16.5});
...
function runTour(){
  ...
  const frame=(ts)=>{
    ...
    _tourRender();
    if(st.frac>=1){ st.done=true; $("tourInstr").textContent="You've arrived — "+(S.destName||"destination"); $("tourDist").textContent=""; $("tourPlay").innerHTML="&#8635;"; return; }
    tourRAF=requestAnimationFrame(frame);
  };
  tourRAF=requestAnimationFrame(frame);
}
function stopTour(){ cancelAnimationFrame(tourRAF); tourState=null; if(tourPuck){try{tourPuck.remove();}catch(e){}tourPuck=null;} tourPins.forEach(p=>{try{p.remove();}catch(e){}}); tourPins=[]; if(tourMap){try{tourMap.remove();}catch(e){}tourMap=null;} $("drivePreview").style.display="none"; }
