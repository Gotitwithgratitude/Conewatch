...
function openSat(lat,lng,name){
  ...
  // container was just shown → measure it, or MapLibre renders black
  satMapObj.on("load",function spin(){
    try{ satMapObj.resize(); }catch(e){}
    setTimeout(function(){ try{ satMapObj&&satMapObj.resize(); satMapObj&&satMapObj.setCenter([lng,lat]); }catch(e){} },260);
...
function _tourRender(){
  ...
  // ═══ G-FORCE LEAN: the camera banks into turns like a car leaning ═══
  var dB=((st.curBrg-prevBrg+540)%360)-180;
  st._lean=(st._lean||0)*0.80 + (-dB*2.6)*0.20;
  var lean=Math.max(-7,Math.min(7,st._lean));
  // ═══ FOV RUSH: pull the view wider as playback speed climbs → world rushes past ═══
  var spd=st.speed||1;
  var zoom=18 - Math.min(0.75,(spd-1)*0.24);
  tourMap.jumpTo({center:ahead,bearing:st.curBrg,pitch:85,zoom:zoom});
...
async function geocodeCandidates(q){
  ...
  var jobs=[
    ...
    fetchT("https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=12&countrycodes=us"+vb+"&q="+encodeURIComponent(q),8000).then(r=>r.json()).then(a=>{out=out.concat(a||[]);}).catch(()=>{}),
