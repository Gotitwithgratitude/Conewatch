...
async function styleFor(theme){
  const url=theme==="light"?"https://tiles.openfreemap.org/styles/liberty":"https://tiles.openfreemap.org/styles/dark";
...
function renderResults(list){
  ...
  if(!list||!list.length){
    ...
    div.innerHTML=`<b>No exact matches</b><small>I can approximate it on the map instead</small>
      <div class="acts"><button class="go">📍 Approximate "${q.slice(0,26)}"</button></div>`;
    div.querySelector(".go").onclick=(e)=>{e.stopPropagation();forceGeocode(q);};
  ...
  list.forEach(r=>{
    ...
    div.querySelector(".go").onclick=(e)=>{e.stopPropagation();box.style.display="none";$("search").value=r.name;
  ...
  if(q2.length>=3){
    const ap=document.createElement("div");ap.className="result";
    ap.innerHTML=`<small>Not seeing your address?</small>
      <div class="acts" style="margin-top:5px"><button class="go" style="background:#2B5FD9;color:#fff">📍 Use "${q2.slice(0,28)}" as typed</button></div>`;
    ap.querySelector(".go").onclick=(e)=>{e.stopPropagation();forceGeocode(q2);};
    box.appendChild(ap);
  }
...
async function forceGeocode(q){
  ...
  const structured=await geocodeStructured(q);
  if(structured){ confirmDestination(structured,q); return; }
  const tries=[]; const loc=await getLocality();
  if(loc)tries.push(`${q}, ${loc}`);
  tries.push(q);
  for(const t of tries){
    try{
      const list=await (await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=10&q=${encodeURIComponent(t)}`)).json();
...
function parseAddr(q){
  var s=q.trim(), out={};
  var zip=s.match(/\b(\d{5})(?:-\d{4})?\b/); if(zip){out.postalcode=zip[1];s=s.replace(zip[0],"");}
  ...
  if(parts.length>=2){
    ...
    if(abbr){out.state=abbr; if(parts.length>=3)out.city=parts[parts.length-2];}
    else out.city=parts[parts.length-1];
  }
  var hn=head.match(/^(\d+[a-z]?)\s+(.*)$/i);
  if(hn){out.housenumber=hn[1];out.street=hn[2];}
...
function pickBest(rows,q){
  ...
  var scored=rows.map(function(r){
    var a=r.address||{}, sc=0;
    if(want.housenumber){
      if(a.house_number===want.housenumber)sc+=100;
      else if(a.house_number)sc-=25;
      else sc-=12;
    }
  ...
  return {lat:+top.r.lat,lng:+top.r.lon,label:top.r.display_name||q,score:top.sc};
}
async function geocodeStructured(q){
  ...
  try{
    var rows=await (await fetch("https://nominatim.openstreetmap.org/search?"+qs)).json();
    return pickBest(rows||[],q);
  }catch(e){ return null; }
}
function confirmDestination(res,typed){
  setDestination({lat:res.lat,lng:res.lng},typed);
  var d=S.pos?distM(S.pos,{lat:res.lat,lng:res.lng}):null;
  $("confAddr").textContent=res.label||typed;
  $("confMeta").textContent=d!==null?fmtDist(d)+" away — about "+Math.max(1,Math.round((d/1609.34)/32*60))+" min drive":"Location found";
  $("confirmBar").style.display="flex";
  clearTimeout(window.__confT); window.__confT=setTimeout(function(){$("confirmBar").style.display="none";},14000);
}
...
async function downloadOfflineArea(){
  ...
  toast("Downloading map for offline use…",2500);
  const cache=await caches.open("cw-tiles-v2");
  const style=await (await fetch("https://tiles.openfreemap.org/styles/liberty")).json().catch(()=>null);
  // derive tile template from the style source
  let tpl=null;
  try{
    const srcs=style&&style.sources||{};
    for(const k in srcs){ if(srcs[k].tiles){tpl=srcs[k].tiles[0];break;}
      if(srcs[k].url){ const tj=await (await fetch(srcs[k].url)).json(); if(tj.tiles)tpl=tj.tiles[0]; break; } }
  }catch(e){}
  if(!tpl)tpl="https://tiles.openfreemap.org/planet/{z}/{x}/{y}.pbf"; // fallback
  const lon=S.pos.lng,lat=S.pos.lat;
  let done=0,total=0,fail=0;
  const jobs=[];
  for(let z=11;z<=16;z++){
    ...
    const r=z<=12?1:z<=14?2:3; // radius of tiles by zoom
    for(let x=cx-r;x<=cx+r;x++)for(let y=cy-r;y<=cy+r;y++){
      if(x<0||y<0||x>=n||y>=n)continue;
      const url=tpl.replace("{z}",z).replace("{x}",x).replace("{y}",y);
      total++;
      jobs.push(fetch(url).then(res=>{if(res.ok)return cache.put(url,res);fail++;}).then(()=>{done++;}).catch(()=>{fail++;done++;}));
    }
  }
  await Promise.all(jobs);
  toast(`✅ Offline area saved (${total-fail} tiles). This region now works with no signal.`,4500);
}
