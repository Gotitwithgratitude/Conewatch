...
function cameraFollow(){
  ...
  if(S.navigating){
    ...
    map.easeTo({center:[S.pos.lng,S.pos.lat],
      zoom:S.navZoom||16.6,pitch:S.saver?0:60,   // saver = flat 2D (far lighter on GPU/battery)
...
function hazardsBlockingAhead(){
  ...
  const BLOCK=["accident","closure","cone","cones","construction","debris"];
...
function poiCategory(q){
  ...
  const K=[[/(food|restaurant|eat|dinner|lunch|hungry)/,"restaurant"],[/(gas|fuel|petrol)/,"fuel"],
    [/(grocery|groceries|supermarket|market)/,"grocery"],[/(coffee|cafe|caf\u00e9)/,"cafe"],
    [/(pharmacy|drugstore|drug store|cvs|walgreens)/,"pharmacy"],[/(hospital|\ber\b|clinic|urgent care)/,"hospital"],
    [/(atm|bank)/,"bank"],[/(hotel|motel|lodging|stay)/,"hotel"],[/(parking|park my car)/,"parking"],
    [/(\bev\b|charger|charging|charge station)/,"charging_station"],[/(attraction|things to do|sightsee)/,"attraction"]];
  for(const [re,key] of K){ if(re.test(s))return Object.assign({key},POI_TAGS[key]); }
  return null;
}
...
function openSat(lat,lng,name){
  ...
  satMapObj=new maplibregl.Map({container:"satMap",
    ...
    center:[lng,lat],zoom:18,pitch:66,bearing:0,maxPitch:85,attributionControl:true});
  ...
  let touched=false;
  const stopSpin=()=>{ touched=true; cancelAnimationFrame(orbitRAF); orbitRAF=null; };
  ["dragstart","zoomstart","rotatestart","pitchstart","touchstart","mousedown","wheel"].forEach(ev=>satMapObj.on(ev,stopSpin));
  satMapObj.on("load",function spin(){
    ...
    const step=()=>{ if(!satMapObj||touched)return;
      // orbit the LOCKED point: keep it centered while the bearing sweeps slowly around it
      satMapObj.setBearing(satMapObj.getBearing()+0.14);
      satMapObj.setCenter([lng,lat]);
...
async function loadPlacePhotos(lat,lng,name){
  ...
  $("photoGrid").innerHTML='<p class="sub" style="grid-column:1/-1">Searching for photos…</p>';
  let imgs=[];
  try{
    const u="https://commons.wikimedia.org/w/api.php?action=query&format=json&origin=*&generator=geosearch&ggscoord="+lat+"|"+lng+"&ggsradius=350&ggslimit=12&prop=imageinfo&iiprop=url&iiurlwidth=420";
    const d=await (await fetch(u)).json();
    const pages=(d.query&&d.query.pages)||{};
    imgs=Object.values(pages).map(p=>p.imageinfo&&p.imageinfo[0]&&p.imageinfo[0].thumburl).filter(Boolean).filter(s=>!/\.(svg|pdf|ogv|webm)$/i.test(s));
  }catch(e){}
  if(!imgs.length){ $("photoGrid").innerHTML='<p class="sub" style="grid-column:1/-1">No public photos found for this exact spot. Landmarks and well-known places have the best coverage — a small business may not be listed.</p>'; return; }
  $("photoGrid").innerHTML="";
  imgs.slice(0,10).forEach(src=>{ const im=document.createElement("img"); im.src=src; im.loading="lazy"; im.referrerPolicy="no-referrer"; im.style.cssText="width:100%;height:120px;object-fit:cover;border-radius:10px;display:block;background:#eee"; $("photoGrid").appendChild(im); });
}
...
function openDriveTour(){
  ...
  tourMap=new maplibregl.Map({container:"driveMap",
    style:{version:8,
      sources:{
        sat:{type:"raster",tiles:satTiles(),tileSize:satMeta().size,attribution:satMeta().attr},
    ...
    center:co[0],zoom:16.9,pitch:76,bearing:_brg(co[0],co[1]),maxPitch:85,attributionControl:true,interactive:true});
  tourMap.on("load",()=>{
    try{ tourMap.setTerrain({source:"dem",exaggeration:1.05}); }catch(e){}
    ...
    // moving puck
    const pk=document.createElement("div"); pk.style.cssText="width:0;height:0;border-left:11px solid transparent;border-right:11px solid transparent;border-bottom:26px solid #2b6cff;filter:drop-shadow(0 2px 4px rgba(0,0,0,.6))";
    try{ tourPuck=new maplibregl.Marker({element:pk,rotationAlignment:"map"}).setLngLat(co[0]).addTo(tourMap); }catch(e){}
    // start/finish flags
...
function startTour(co,cum,total,marks){
  ...
  tourState={co,cum,total,marks,baseDur,frac:0,speed:1,paused:false,done:false,curBrg:_brg(co[0],co[1])};
...
function _tourRender(){
  ...
  const pos=_posAt(st.co,st.cum,d);
  const tgt=_brgAt(st.co,st.cum,d);
  // bearing smoothing = stabilization: ease into turns instead of snapping
  st.curBrg=_lerpAng(st.curBrg,tgt,0.12);
  tourMap.jumpTo({center:pos,bearing:st.curBrg,pitch:76,zoom:16.9});
...
function arriveCinematic(){
  ...
  try{
    ...
    tourMap.easeTo({center:end,bearing:brg,pitch:68,zoom:18.7,duration:2800,essential:true});
...
function stopTour(){ cancelAnimationFrame(arriveRAF); arriveRAF=null; cancelAnimationFrame(tourRAF); tourState=null; if(tourPuck){try{tourPuck.remove();}catch(e){}tourPuck=null;} tourPins.forEach(p=>{try{p.remove();}catch(e){}}); tourPins=[]; if(tourMap){try{tourMap.remove();}catch(e){}tourMap=null;} $("drivePreview").style.display="none"; }
...
$("tourSpeed")&&($("tourSpeed").onclick=()=>{ const st=tourState; if(!st)return; st.speed=st.speed===1?2:st.speed===2?4:1; $("tourSpeed").innerHTML=st.speed+"&times;"; });
...
async function geocodeCandidates(q){
  ...
  var jobs=[
    fetchT("https://nominatim.openstreetmap.org/search?"+qs,8000).then(r=>r.json()).then(a=>{out=out.concat(a||[]);}).catch(()=>{}),
    fetchT("https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=10&countrycodes=us&q="+encodeURIComponent(q),8000).then(r=>r.json()).then(b=>{out=out.concat(b||[]);}).catch(()=>{}),
    fetchT("https://photon.komoot.io/api/?limit=10&lang=en&q="+encodeURIComponent(q),8000).then(r=>r.json()).then(d=>{out=out.concat(photonToRows(d.features||[]));}).catch(()=>{})
  ];
...
function showGeoPicker(cands,typed){
  ...
  $("geoPickerSub").textContent="Tap the correct address for \u201C"+typed+"\u201D";
...
setTimeout(()=>{ if(isIOSdev()&&!isStandalone()&&!installDismissed())showInstallBanner(); },3800);
