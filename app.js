...
async function reportHazard(type,note){
  ...
  if(near){
    ...
    if(S.sb.url&&S.sb.key){ try{ await fetch(`${S.sb.url}/rest/v1/hazards`,{method:"POST",headers:{"Content-Type":"application/json",apikey:S.sb.key,Authorization:`Bearer ${S.sb.key}`},body:JSON.stringify({type,lat:S.pos.lat,lng:S.pos.lng,note:note||"confirm",sev:2,reports:1})}); }catch(e){} }
    return;
  }
