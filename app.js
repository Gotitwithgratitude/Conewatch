...
async function reportHazard(type,note){
  ...
  if(S.sb.url&&S.sb.key){
    try{
      ...
      const r=await fetch(`${S.sb.url}/rest/v1/hazards`,{method:"POST",headers:{"Content-Type":"application/json",apikey:S.sb.key,Authorization:`Bearer ${S.sb.key}`,Prefer:"return=minimal"},body:JSON.stringify(payload)});
...
async function crowdLookup(q){
  ...
  try{
    ...
    const rows=await (await fetch(`${S.sb.url}/rest/v1/geo_picks?q=eq.${encodeURIComponent(key)}&select=lat,lng,label&limit=50`,{headers:{apikey:S.sb.key,Authorization:`Bearer ${S.sb.key}`}})).json();
...
function crowdSave(q,res){
  ...
  try{ fetch(`${S.sb.url}/rest/v1/geo_picks`,{method:"POST",headers:{"Content-Type":"application/json",apikey:S.sb.key,Authorization:`Bearer ${S.sb.key}`},body:JSON.stringify({q:q.trim().toLowerCase(),lat:res.lat,lng:res.lng,label:res.label||q})}); }catch(e){}
}
