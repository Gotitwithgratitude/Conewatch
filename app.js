...
async function loadPlacePhotos(lat,lng,name){
  ...
  // C) Wikimedia Commons by COORDINATES (catches unnamed nearby photos)
  jobs.push(fetch("https://commons.wikimedia.org/w/api.php?action=query&format=json&origin=*&generator=geosearch&ggscoord="+lat+"|"+lng+"&ggsradius=500&ggslimit=10&prop=imageinfo&iiprop=url&iiurlwidth=420")
  ...
  if(!imgs.length){ $("photoGrid").innerHTML='<p class="sub" style="grid-column:1/-1">No public photos found for this place. Well-known spots have the best coverage.</p>'; return; }
  $("photoGrid").innerHTML="";
