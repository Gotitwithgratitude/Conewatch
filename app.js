...
async function forceGeocode(q){
  ...
  // 1) try the raw typed address with structured parsing (most accurate)
  const structured=await geocodeStructured(q);
  if(structured && structured.score>=60){ confirmDestination(structured,q); return; }
  // 2) if weak/no match AND user gave no city, retry with their area appended
  const p0=parseAddr(q);
  let best=structured;
  if(!p0.city && !p0.state && !p0.postalcode){
    const loc=await getLocality();
    if(loc){ const s2=await geocodeStructured(q+", "+loc);
