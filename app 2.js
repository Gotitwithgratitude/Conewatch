...
async function forceGeocode(q){
  ...
  // if user typed no city/state/zip, append their current area automatically
  const p0=parseAddr(q);
  if(!p0.city && !p0.state && !p0.postalcode){
    const loc=await getLocality();
    if(loc){ const q2=q+", "+loc;
      const s2=await geocodeStructured(q2); if(s2){confirmDestination(s2,q);return;}
    }
  }
...
async function confirmDestination(res,typed){
  ...
  if(S.pos&&navigator.onLine){
    try{
      const u=`https://routing.openstreetmap.de/routed-car/route/v1/driving/${S.pos.lng},${S.pos.lat};${res.lng},${res.lat}?overview=false`;
      let data=await (await fetch(u)).json();
      if(data.code!=="Ok"){ data=await (await fetch(`https://router.project-osrm.org/route/v1/driving/${S.pos.lng},${S.pos.lat};${res.lng},${res.lat}?overview=false`)).json(); }
